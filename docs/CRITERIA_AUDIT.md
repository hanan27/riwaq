...
| Cost/latency | 15 | Every model attempt metered; exact-cache isolation; eight near misses; executed before/after with eval beside every row | Provider cached-input ratio ≥65%; real ≥60% dollar savings; measured semantic-cache threshold |
...
| Complete application | 10 | Self-contained notebook; four-part bilingual demo; fresh-process execution with outputs | Fresh Colab browser Run all; final identity/cohort metadata; repository URL and genuine incremental history |
| Extension | up to +5 | Five poisoned output cases and negative tool tests | Extension points only apply if mandatory scope scores ≥80; do not assume them |
